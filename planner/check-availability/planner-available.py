...
url_future = "https://myplanner.netcompany-intrasoft.com/deskbooking/api/v1/deskbooking/future?page=0&size=15"
...
def make_future_request():
    """Make a GET request for a specific date."""
    response_future = requests.get(url_future, headers=headers)
...
def main():
    data_future = make_future_request()
    ...
    while True:
        ...
        for date in days:
            ...
            if data:
                ...
                # specific floor
                #----------------------------------------
                if (search_item not in data_future):
                    ...
                    if (data['floor'] == "1st Floor"  or data['floor'] =="Mezzazine"):
                        ...
                        webbrowser.open_new("https://myplanner.netcompany-intrasoft.com/booking/create")
                        days.remove(date)
                        EmailSender.send_email(message_title, message_txt)
                else:
                    #already booked. no search again
                    print(f"Date {date} will be removed from dates array \n\n")
                    days.remove(date)
            else:
